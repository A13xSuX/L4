...
func BenchmarkGetOrderCacheHit(b *testing.B) {
	...
	testOrder := database.Order{
		OrderUID:    "test_bench",
		TrackNumber: "FROM_CACHE",
	}
	cache.Set(testOrder)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		order, err := service.GetOrder(testOrder.OrderUID)
		if err != nil {
			b.Fatalf("Failed to get order: %v", err)
		}
		if order.TrackNumber != "FROM_CACHE" {
			b.Fatalf("Expected track number 'FROM_CACHE', but got %s", order.TrackNumber)
		}
	}
}
...
func makeValidOrder() database.Order {
